...
class ConvBlock(nn.Module):
    ...
    def transform_input_lengths(length, module):
        ...
        if type(module) is not nn.Conv2d:
            ...
            return (length + 2 * module.padding[1] - module.dilation[1] * (module.kernel_size[1] - 1) - 1) // \
                module.stride[1] + 1
...
class RNNBlock(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, rnn_type, batch_first=True, dropout=0.0, bidirectional=True,
                 batch_norm=True):
        """
            Args:
        ...
        self.rnn = getattr(torch.nn, rnn_type)(input_size, hidden_size, batch_first=batch_first, dropout=dropout,
...
class DeepSpeech2(nn.Module):
    ...
    def __init__(self, n_feats, n_tokens, fc_hidden, num_rnn, dropout, rnn_type: str, bidirectional=True):
        """
            Args:
        ...
        self.gru = nn.Sequential(
            RNNBlock(input_size * 96, fc_hidden, rnn_type, True, dropout, bidirectional, False)
            ,
            *[RNNBlock(fc_hidden, fc_hidden, rnn_type, True, dropout, bidirectional, True) for _ in range(num_rnn - 1)]
        )
    ...
    def forward(self, spectrogram, spectrogram_length, **batch):
        """
            ...
            Returns:
                output (dict): output dict containing logits and
                    transformed lengths.
        """
        ...
        return {'logits': self.fc(x), 'length': length}
